...
def save_image(reshaped_data, filename):
    """
    Save the mass spectrometry data as a heatmap grid.
    """
...
def display_image_with_colorbar(reshaped_data):
    """
    Display the mass spectrometry data as a heatmap grid with a colorbar.
    """
    image = np.array(reshaped_data)
    plt.imshow(image, cmap="hot", aspect='auto')
...
def main():
    ...
    # Step 8: Save the mass spectrometry image as a heatmap
    save_image(reversed_data, 'MSI_heatmap.png')

    # Step 9: Display the mass spectrometry image as a heatmap with colorbar
